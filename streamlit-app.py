...
if 'accent_color' not in st.session_state:
    st.session_state.accent_color = 'blue'
...
# Get current theme and accent colors
current_theme = themes[st.session_state.theme]
current_accent = accent_colors[st.session_state.accent_color]
...
primary_color = current_accent['primary']
secondary_color = current_accent['secondary']
accent_color = current_accent['accent']
dark_color = current_theme['secondary_bg']
light_color = current_theme['secondary_text']

# Function to get base64 encoded image for background patterns
def get_base64_pattern():
    pattern_svg = f'''
    <svg width="100" height="100" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
        <circle cx="10" cy="10" r="2" fill="{current_accent['primary']}20" />
        <circle cx="30" cy="30" r="2" fill="{current_accent['primary']}20" />
        <circle cx="50" cy="50" r="2" fill="{current_accent['primary']}20" />
        <circle cx="70" cy="70" r="2" fill="{current_accent['primary']}20" />
        <circle cx="90" cy="90" r="2" fill="{current_accent['primary']}20" />
    </svg>
    '''
    return base64.b64encode(pattern_svg.encode()).decode()

# Function to create tooltip HTML
def create_tooltip(tooltip_id, content):
    if st.session_state.show_tooltips:
        return f'''
        <div class="tooltip-container">
            <span class="tooltip-icon" id="{tooltip_id}">ⓘ</span>
...
st.markdown("""
    <style>
        /* Base Theme Styling */
        :root {
            --primary-color: """ + current_accent['primary'] + """;
            --secondary-color: """ + current_accent['secondary'] + """;
            --accent-color: """ + current_accent['accent'] + """;
            --highlight-color: """ + current_accent['highlight'] + """;
            --warning-color: """ + current_accent['warning'] + """;
            --error-color: """ + current_accent['error'] + """;
            --success-color: """ + current_accent['success'] + """;
            ...
            --font-family: """ + st.session_state.font + """, -apple-system, BlinkMacSystemFont, sans-serif;
        }

        /* Global Styles */
        .stApp {{
            background-color: var(--bg-color);
            background-image: url("data:image/svg+xml;base64,""" + get_base64_pattern() + """");
        ...
        .dashboard-card, .metric-card, .info-text {{
...
def save_to_history(analysis_type, description):
...
def export_dataframe(df, filename):
    ...
    href = f'<a href="data:file/csv;base64,{b64}" download="{filename}.csv" class="export-button"><span class="export-icon">📊</span>Download CSV</a>'
    return href

# Function to create a comparison view
def create_comparison_view(theoretical, simulated, metrics):
    html = """
    <div class="comparison-card">
        <div class="comparison-side">
            <div class="comparison-title">Theoretical</div>
    """
    for metric in metrics:
        html += f"<p><strong>{metric}:</strong> {theoretical[metric]:.4f}</p>"

    html += """
        </div>
        <div class="comparison-side">
            <div class="comparison-title">Simulated</div>
    """
    for metric in metrics:
        html += f"<p><strong>{metric}:</strong> {simulated[metric]:.4f}</p>"
...
def main():
    ...
    if st.session_state.first_visit:
        ...
        st.info("👋 Welcome to the enhanced Stochastic Process Analyzer! Explore the new features in the sidebar.")
    ...
    with st.sidebar:
        ...
        st.markdown(f"""
        <div style="padding: 10px; background-color: var(--secondary-bg); border-radius: 10px; margin-bottom: 20px;">
            <div style="display: flex; align-items: center;">
                <div style="width: 40px; height: 40px; border-radius: 50%; background-color: var(--primary-color);
                     display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; margin-right: 10px;">
                    {st.session_state.user_id[:2].upper()}
                </div>
                <div>
                    <div style="font-weight: bold;">User Session</div>
                    <div style="font-size: 0.8rem; color: var(--secondary-text);">ID: {st.session_state.user_id[:8]}...</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        app_mode = st.radio("",
                          ...
                          format_func=lambda x: f"{'🏠' if x=='Home' else '🔄' if x=='Markov Chain Analysis' else '🔍' if x=='Hidden Markov Model Analysis' else '🏦' if x=='Queuing Theory Analysis' else '🎮'} {x}")
        ...
        with st.expander("Appearance"):
            ...
            with accent_col:
                selected_accent = st.selectbox("Accent", list(accent_colors.keys()), index=list(accent_colors.keys()).index(st.session_state.accent_color))
                if selected_accent != st.session_state.accent_color:
                    st.session_state.accent_color = selected_accent
                    st.experimental_rerun()

            font = st.selectbox("Font", ["sans-serif", "serif", "monospace"], index=["sans-serif", "serif", "monospace"].index(st.session_state.font))
            if font != st.session_state.font:
                st.session_state.font = font
                st.experimental_rerun()

        with st.expander("Preferences"):
            st.checkbox("Show tooltips", value=st.session_state.show_tooltips, key="show_tooltips_toggle",
                       on_change=lambda: setattr(st.session_state, "show_tooltips", st.session_state.show_tooltips_toggle))

            animation_speed = st.select_slider("Animation speed", ["slow", "normal", "fast"], value=st.session_state.animation_speed)
            if animation_speed != st.session_state.animation_speed:
                st.session_state.animation_speed = animation_speed
                st.experimental_rerun()

            st.checkbox("Advanced mode", value=st.session_state.advanced_mode, key="advanced_mode_toggle",
                       on_change=lambda: setattr(st.session_state, "advanced_mode", st.session_state.advanced_mode_toggle))
        ...
        if st.session_state.history:
            ...
            for i, item in enumerate(reversed(st.session_state.history)):
                st.markdown(f"""
                <div class="history-item" onclick="alert('Loading previous analysis is not implemented in this demo')">
        ...
        with st.expander("Keyboard Shortcuts"):
            st.markdown("""
            - <span class="keyboard-shortcut">Ctrl</span> + <span class="keyboard-shortcut">H</span> Toggle sidebar
            - <span class="keyboard-shortcut">Ctrl</span> + <span class="keyboard-shortcut">T</span> Toggle theme
            """, unsafe_allow_html=True)

        # Footer
        st.markdown("""
        <div style="position: fixed; bottom: 0; left: 0; width: 100%; background-color: var(--sidebar-bg); padding: 10px; text-align: center; font-size: 0.8rem; color: var(--secondary-text);">
            Stochastic Process Analyzer v2.0
        </div>
        """, unsafe_allow_html=True)
    ...
    if app_mode == "Home":
        ...
        st.markdown("""
        <div style="text-align: center; padding: 40px 0; animation: fadeIn 1s ease-out;">
            <h1 style="font-size: 3rem; margin-bottom: 10px;">Stochastic Process Analyzer</h1>
            <p style="font-size: 1.2rem; color: var(--secondary-text); margin-bottom: 30px;">
                Advanced analysis and visualization of stochastic processes
            </p>
            <div style="width: 100px; height: 5px; background: linear-gradient(90deg, var(--primary-color), var(--secondary-color));
                 margin: 0 auto 40px auto; border-radius: 5px;"></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with stats_col1:
            ...
            <div class="metric-card card-3d">
                <div class="card-3d-inner">
                    ...
                    <div style="font-size: 0.9rem;">Available modules</div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with stats_col2:
            ...
            <div class="metric-card card-3d">
                <div class="card-3d-inner">
                    ...
                    <div style="font-size: 0.9rem;">Interactive charts</div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with stats_col3:
            ...
            <div class="metric-card card-3d">
                <div class="card-3d-inner">
                    ...
                    <div style="font-size: 0.9rem;">Mathematical models</div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with stats_col4:
            ...
            <div class="metric-card card-3d">
                <div class="card-3d-inner">
                    <div class="metric-label">THEME OPTIONS</div>
                    <div class="metric-value">16</div>
                    <div style="font-size: 0.9rem;">Customization combinations</div>
                </div>
            </div>
            """, unsafe_allow_html=True)

        # Main features section
        st.markdown("<h2>Analysis Modules</h2>", unsafe_allow_html=True)

        # Create tabs for each analysis type
        module_tabs = st.tabs(["🔄 Markov Chains", "🔍 Hidden Markov Models", "🏦 Queuing Theory", "🎮 Simulation"])
        ...
        with module_tabs[0]:
            ...
            with col1:
                ...
                <div class="dashboard-card">
                    ...
                    <p>Analyze discrete state transitions with the Markov property:</p>
                    <ul>
                        <li>Calculate transition matrices and visualize state diagrams</li>
                        <li>Compute steady-state probabilities and limiting distributions</li>
                        <li>Analyze mean first passage times and recurrence times</li>
                        <li>Identify absorbing states and calculate absorption probabilities</li>
                        <li>Determine state classifications and periodicity</li>
                    </ul>
                    <p>Perfect for analyzing systems that transition between discrete states over time.</p>
                </div>
                """, unsafe_allow_html=True)

            with col2:
                st.image("https://upload.wikimedia.org/wikipedia/commons/thumb/2/2b/Markovkate_01.svg/500px-Markovkate_01.svg.png",
                        caption="Example Markov Chain Diagram")

                st.markdown("""
                <div style="text-align: center; margin-top: 20px;">
                    <a href="#" onclick="window.open('https://en.wikipedia.org/wiki/Markov_chain')" class="export-button">
                        Learn More
                    </a>
                </div>
                """, unsafe_allow_html=True)
        ...
        with module_tabs[1]:
            ...
            with col1:
                ...
                <div class="dashboard-card">
                    ...
                    <p>Analyze systems with hidden states that generate observable events:</p>
                    <ul>
                        <li>Calculate transition and emission probabilities</li>
                        <li>Apply the Forward algorithm to compute observation probabilities</li>
                        <li>Use the Viterbi algorithm to find the most likely hidden state sequence</li>
                        <li>Visualize hidden state transitions and emission patterns</li>
                        <li>Compute joint and marginal probabilities</li>
                    </ul>
                    <p>Ideal for speech recognition, biological sequence analysis, and more.</p>
                </div>
                """, unsafe_allow_html=True)

            with col2:
                st.image("https://upload.wikimedia.org/wikipedia/commons/thumb/8/83/HiddenMarkovModel.svg/500px-HiddenMarkovModel.svg.png",
                        caption="Hidden Markov Model Structure")

                st.markdown("""
                <div style="text-align: center; margin-top: 20px;">
                    <a href="#" onclick="window.open('https://en.wikipedia.org/wiki/Hidden_Markov_model')" class="export-button">
                        Learn More
                    </a>
                </div>
                """, unsafe_allow_html=True)
        ...
        with module_tabs[2]:
            ...
            with col1:
                st.markdown("""
                <div class="dashboard-card">
                    <h3>Queuing Theory Analysis</h3>
                    <p>Analyze waiting line systems with M/M/s queuing models:</p>
                    <ul>
                        <li>Calculate arrival and service rates from empirical data</li>
                        <li>Determine system utilization and stability</li>
                        <li>Compute average queue lengths and waiting times</li>
                        <li>Analyze probability distributions of system states</li>
                        <li>Verify Little's Law and other queuing relationships</li>
                    </ul>
                    <p>Essential for optimizing service systems, call centers, and resource allocation.</p>
                </div>
                """, unsafe_allow_html=True)

            with col2:
                st.image("https://upload.wikimedia.org/wikipedia/commons/thumb/9/91/Mm1_queue.svg/500px-Mm1_queue.svg.png",
                        caption="M/M/1 Queuing System")

                st.markdown("""
                <div style="text-align: center; margin-top: 20px;">
                    <a href="#" onclick="window.open('https://en.wikipedia.org/wiki/Queueing_theory')" class="export-button">
                        Learn More
                    </a>
                </div>
                """, unsafe_allow_html=True)
        ...
        with module_tabs[3]:
            ...
            with col1:
                st.markdown("""
                <div class="dashboard-card">
                    <h3>Stochastic Process Simulation</h3>
                    <p>Run dynamic simulations of various stochastic processes:</p>
                    <ul>
                        <li>Simulate Markov chains with custom transition matrices</li>
                        <li>Generate hidden state and observation sequences from HMMs</li>
                        <li>Run discrete-event simulations of queuing systems</li>
                        <li>Compare empirical results with theoretical predictions</li>
                        <li>Visualize system behavior over time with interactive plots</li>
                    </ul>
                    <p>Perfect for understanding system dynamics and validating analytical models.</p>
                </div>
                """, unsafe_allow_html=True)

            with col2:
                st.image("https://upload.wikimedia.org/wikipedia/commons/thumb/7/7a/Random_walk_2500_animated.svg/500px-Random_walk_2500_animated.svg.png",
                        caption="Stochastic Process Simulation")

                st.markdown("""
                <div style="text-align: center; margin-top: 20px;">
                    <a href="#" onclick="window.open('https://en.wikipedia.org/wiki/Stochastic_process')" class="export-button">
                        Learn More
                    </a>
                </div>
                """, unsafe_allow_html=True)
        ...
        with start_col1:
            ...
            <div class="dashboard-card">
                ...
                <p>To analyze your data, you'll need to upload a CSV file with the appropriate columns for each analysis type:</p>

                <h4>Markov Chain Data</h4>
                <p>CSV with <code>current_state</code> and <code>next_state</code> columns</p>

                <h4>Hidden Markov Model Data</h4>
                <p>CSV with <code>hidden_state</code> and <code>observed_event</code> columns</p>

                <h4>Queuing Theory Data</h4>
                <p>CSV with <code>arrival_time_minutes</code> and <code>service_time_minutes</code> columns</p>

                <p>Don't have data? Use our example datasets or manual entry options.</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with start_col2:
            ...
            <div class="dashboard-card">
                ...
                <ol>
                    <li><strong>Select an analysis type</strong> from the navigation menu</li>
                    <li><strong>Upload your data</strong> or use example data</li>
                    <li><strong>Configure parameters</strong> specific to your analysis</li>
                    <li><strong>Run the analysis</strong> and explore the results</li>
                    <li><strong>Export results</strong> for further use or reporting</li>
                </ol>

                <p>Need more help? Click the tooltips <span style="display: inline-flex; align-items: center; justify-content: center; width: 18px; height: 18px; border-radius: 50%; background-color: var(--primary-color); color: white; font-size: 12px;">ⓘ</span> throughout the app for detailed explanations.</p>
            </div>
            """, unsafe_allow_html=True)

        # Sample data formats in expandable section
        with st.expander("View Sample Data Formats"):
            format_tabs = st.tabs(["Markov Chain", "Hidden Markov Model", "Queuing Theory"])
            ...
            with format_tabs[0]:
                ...
                st.download_button(
                    ...
                    file_name="markov_chain_sample.csv",
                    mime="text/csv"
                )
            ...
            with format_tabs[2]:
                ...
                st.download_button(
                    ...
                    file_name="queue_sample.csv",
                    mime="text/csv"
                )

        # Footer with version info
        st.markdown("""
        <div style="margin-top: 50px; text-align: center; padding: 20px; border-top: 1px solid var(--secondary-bg);">
            <p style="color: var(--secondary-text); font-size: 0.9rem;">
                Stochastic Process Analyzer v2.0 | Enhanced UI Edition | Built with Streamlit
            </p>
        </div>
        """, unsafe_allow_html=True)
